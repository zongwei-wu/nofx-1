...
// traderExchangePositionOverlays 从交易所实时持仓构建 K 线成本线（与 positions API 口径一致）
func (s *Server) traderExchangePositionOverlays(traderID, symbolFilter string) ([]map[string]interface{}, error) {
	...
	for _, p := range positions {
		...
		if symbolFilter != "" && symbol != normalizeTradeSymbol(symbolFilter) {
			continue
		}
...
func (s *Server) handleTradeEvents(c *gin.Context) {
	...
	case "ai_copy_trade":
		...
		if ovs, err := s.traderExchangePositionOverlays(traderID, symbol); err == nil {
