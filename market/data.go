...
func Get(symbol string) (*Data, error) {
	...
	ik3m := toIndicatorKlines(klines3m)
	currentEMA20 := indicator.EMA(ik3m, 20).Current
	currentMACD := indicator.MACDLine(ik3m)
	currentRSI7 := indicator.RSI(ik3m, 7).Current
...
// toIndicatorKlines 将 market.Kline 转为 indicator.Kline
func toIndicatorKlines(klines []Kline) []indicator.Kline {
...
func calculateIntradaySeries(klines []Kline) *IntradayData {
	...
	for i := start; i < len(klines); i++ {
		...
		// 计算每个点的EMA20
		if i >= 19 {
			ema20 := indicator.EMA(toIndicatorKlines(klines[:i+1]), 20).Current
			data.EMA20Values = append(data.EMA20Values, ema20)
		}

		// 计算每个点的MACD
		if i >= 25 {
			macd := indicator.MACDLine(toIndicatorKlines(klines[:i+1]))
			data.MACDValues = append(data.MACDValues, macd)
		}

		// 计算每个点的RSI
		if i >= 7 {
			rsi7 := indicator.RSI(toIndicatorKlines(klines[:i+1]), 7).Current
			data.RSI7Values = append(data.RSI7Values, rsi7)
		}
		if i >= 14 {
			rsi14 := indicator.RSI(toIndicatorKlines(klines[:i+1]), 14).Current
			data.RSI14Values = append(data.RSI14Values, rsi14)
		}
	}

	// 计算3m ATR14
	data.ATR14 = indicator.ATR(toIndicatorKlines(klines), 14).Current
...
func calculateLongerTermData(klines []Kline) *LongerTermData {
	...
	ik := toIndicatorKlines(klines)
	// 计算EMA
	data.EMA20 = indicator.EMA(ik, 20).Current
	data.EMA50 = indicator.EMA(ik, 50).Current

	// 计算ATR
	data.ATR3 = indicator.ATR(ik, 3).Current
	data.ATR14 = indicator.ATR(ik, 14).Current
